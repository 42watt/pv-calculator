  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" preserveAspectRatio="xMidYMid meet">
        {yLabels.map((label, i) => (
          <line key={i} x1={padLeft} x2={width - padRight} y1={label.y} y2={label.y} stroke="#e5e5e5" strokeWidth="0.15" />
        ))}
        ...
        {data.map((d, i) => (
          <circle key={i} cx={toX(i)} cy={toY(d.cumulativeSavings)} r="0.6" fill="var(--color--light-blue)" />
        ))}

        {data.filter((_, i) => i % Math.ceil(data.length / 10) === 0 || i === data.length - 1).map((d) => (
          <text key={d.year} x={toX(data.indexOf(d))} y={height - 1} textAnchor="middle" fontSize="2.2" fill="var(--color--dark-grey)">
            {d.year}
          </text>
        ))}

        {yLabels.filter((_, i) => i % 2 === 0 || i === yLabels.length - 1).map((label, i) => (
          <text key={i} x={padLeft + 0.5} y={label.y - 0.5} fontSize="1.8" fill="var(--color--dark-grey)">
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 border-2 border-[var(--color--medium-grey)] rounded-lg focus:border-[var(--color--light-blue)] focus:outline-none"
                  >
                ...
                <div className="border-t border-[var(--color--medium-grey)] pt-5">
                  <button
                    ...
                    className="w-full flex items-center justify-between px-4 py-3 bg-[var(--color--light-grey)] rounded-lg hover:bg-[var(--color--beige)] transition-colors text-sm font-semibold text-[var(--color--black)]"
                  >
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-4 py-2.5 border-2 border-[var(--color--medium-grey)] rounded-lg focus:border-[var(--color--light-blue)] focus:outline-none pr-16"
                          />
                          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[var(--color--dark-grey)]">€/Jahr</span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-4 py-2.5 border-2 border-[var(--color--medium-grey)] rounded-lg focus:border-[var(--color--light-blue)] focus:outline-none pr-8"
                          />
                          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[var(--color--dark-grey)]">€</span>
                        </div>
                        <p className="text-xs text-[var(--color--dark-grey)] mt-1">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white rounded-2xl shadow-lg p-5 text-center">
                <div className="text-sm text-[var(--color--dark-grey)] mb-1">Amortisation</div>
                ...
                <div className="text-xs text-[var(--color--dark-grey)] mt-1">bis die Mehrkosten sich rechnen</div>
              </div>

              <div className="bg-white rounded-2xl shadow-lg p-5 text-center">
                <div className="text-sm text-[var(--color--dark-grey)] mb-1">Ersparnis Jahr 1</div>
                <div className={`text-3xl md:text-4xl font-bold ${annualSavingsYear1 >= 0 ? 'text-[var(--color--green)]' : 'text-red-500'}`}>
                  {annualSavingsYear1.toLocaleString('de-DE')} €
                </div>
                <div className="text-xs text-[var(--color--dark-grey)] mt-1">Betriebskosten pro Jahr</div>
              </div>

              <div className="bg-white rounded-2xl shadow-lg p-5 text-center">
                <div className="text-sm text-[var(--color--dark-grey)] mb-1">Nettogewinn</div>
                <div className={`text-3xl md:text-4xl font-bold ${results.totalSavings >= 0 ? 'text-[var(--color--green)]' : 'text-red-500'}`}>
                  {results.totalSavings.toLocaleString('de-DE')} €
                </div>
                <div className="text-xs text-[var(--color--dark-grey)] mt-1">nach {expert.observationPeriod} Jahren</div>
              </div>

              <div className="bg-white rounded-2xl shadow-lg p-5 text-center">
                <div className="text-sm text-[var(--color--dark-grey)] mb-1">CO₂ eingespart</div>
                <div className="text-3xl md:text-4xl font-bold text-[var(--color--light-blue)]">
                  {results.co2Saved.toLocaleString('de-DE')} t
                </div>
                <div className="text-xs text-[var(--color--dark-grey)] mt-1">über {expert.observationPeriod} Jahre</div>
              </div>
            </div>

            {/* Comparison Summary */}
            <div className="bg-[var(--color--dark-blue)] text-white rounded-2xl shadow-lg p-6">
              <h3 className="text-lg font-bold mb-4">Kostenvergleich auf {expert.observationPeriod} Jahre</h3>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <div className="text-sm opacity-70 mb-1">{systemLabels[inputs.oldSystem]}</div>
                ...
                <div>
                  <div className="text-sm opacity-70 mb-1">Wärmepumpe</div>
              ...
              <div className="mt-4 pt-4 border-t border-white border-opacity-20">
                <div className="flex justify-between items-center">
                  <span className="text-sm">Mehrinvestition WP</span>
                  <span className="font-bold">{results.investmentDifference.toLocaleString('de-DE')} €</span>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-[var(--color--medium-grey)]">
                      ...
                      <th className="text-right py-2 px-2 font-semibold">{inputs.oldSystem === 'gas' ? 'Gas' : inputs.oldSystem === 'oil' ? 'Öl' : inputs.oldSystem === 'nachtspeicher' ? 'Nachts.' : 'Fernw.'}</th>
                      <th className="text-right py-2 px-2 font-semibold">Wärmepumpe</th>
                      <th className="text-right py-2 px-2 font-semibold">Ersparnis/Jahr</th>
                      <th className="text-right py-2 px-2 font-semibold">Kum. Gewinn</th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.yearly.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="text-right py-2 px-2">{row.oldCosts.toLocaleString('de-DE')} €</td>
                        <td className="text-right py-2 px-2">{row.wpCosts.toLocaleString('de-DE')} €</td>
                        <td className={`text-right py-2 px-2 font-semibold ${row.savings >= 0 ? 'text-[var(--color--green)]' : 'text-red-500'}`}>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="space-y-3 text-sm text-[var(--color--black)]">
                <div className="flex gap-3 items-start">
                  <span className="text-[var(--color--light-blue)] font-bold mt-0.5 flex-shrink-0">2025</span>
                  <span>CO₂-Preis: 55 €/t → Aufschlag Gas: 1,32 ct/kWh, Öl: 1,74 ct/kWh</span>
                </div>
                <div className="flex gap-3 items-start">
                  <span className="text-[var(--color--light-blue)] font-bold mt-0.5 flex-shrink-0">2026+</span>
                  <span>CO₂-Preiskorridor: 55-65 €/t, danach marktbasiert (EU-ETS 2)</span>
                </div>
                <div className="flex gap-3 items-start">
                  <span className="text-[var(--color--light-blue)] font-bold mt-0.5 flex-shrink-0">2029</span>
                  <span>Biogas-/Bio-Öl-Beimischungspflichten treten in Kraft</span>
                </div>
                <div className="flex gap-3 items-start">
                  <span className="text-red-500 font-bold mt-0.5 flex-shrink-0">2045</span>
                  <span>Betriebsverbot für fossile Heizungen (GEG) – begrenzt die Nutzungsdauer einer heute eingebauten Gas-/Ölheizung</span>
                </div>
              </div>
              <div className="mt-4 p-3 bg-[var(--color--light-grey)] rounded-lg">
